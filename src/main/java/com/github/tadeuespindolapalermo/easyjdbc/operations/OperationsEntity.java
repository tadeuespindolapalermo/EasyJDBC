...
public class OperationsEntity<T> {
	...
	protected String defineTableName(Class<?> entity) {
		return !verifyEntityName(entity).isEmpty() ? verifyEntityName(entity) : entity.getSimpleName().toLowerCase();
	}
	...
	protected void processSelect(String query, List<T> entities) throws SQLException, NoSuchMethodException,
		...
		try (PreparedStatement stmt = connection.prepareStatement(query)) {
			for (int i = 0; i <= fields.length - 1; i++) {
				...
				if (!(idAutoIncrement && fields[i].getName().equals(getIdName(entity.getClass()))))
					setStatementProcess(entity, stmt, i, getMethodGetter(entityClass, i, fields).invoke(entity),
							operation, idAutoIncrement, fields);
			}
			stmt.executeUpdate();
			connection.commit();
		}
	...
	private void setStatementProcess(T entity, PreparedStatement stmt, int i, Object value, String operation,
			boolean idAutoIncrement, Field[] fields) throws SQLException {

		if (operation.equals(UPDATE) && fields[i].getName().equals(getIdName(entity.getClass())) && !idAutoIncrement) {
			flag = true;
			return;
		}
	...
	private void setStatement(PreparedStatement stmt, int i, Object value, int index, Field[] fields)
			throws SQLException {
	...
	protected String mountQueryInsert(String table, Field[] fields, boolean idAutoIncrement) {
		...
		StringBuilder sql = new StringBuilder(INSERT_INTO).append(table).append(" (" + columnsName + ") ")
	...
	private Field[] removeSerialVersionUIDAttribute(Field[] fields) {
		...
		int fieldsLength = containsAttributeSerialVersionUID(fields) ? fields.length - 1 : fields.length;
		Field[] fieldsWithoutSerialVersionUIDAttribute = new Field[fieldsLength];
		for (int i = 0; i <= fields.length - 1; i++) {
			int indexFieldsWithoutSerialVersionUIDAttribute = flagExistAttributeSerialVersionUID ? i - 1 : i;
	...
	private Field[] removeAttributeNotColumn(Field[] fields) {
		...
		for (int i = 0; i <= fields.length - 1; i++) {
			int indexFieldsWithoutNotColumnAnnotation = flagExistAttributeNotColumn ? i - indexSubtract : i;
	...
	protected String mountQueryUpdate(String table, Field[] fields, Object idValue, String idName) {
		...
		StringBuilder sql = new StringBuilder("UPDATE ").append(table).append(" SET ").append(columnsName.toString())
	...
	protected String mountQueryDelete(String table, Object id) {
		...
		StringBuilder sql = new StringBuilder("DELETE FROM ").append(table).append(WHERE_CLAUSE)
				.append(getIdName(entity)).append(" = ").append("'" + id + "'");
	...
	protected String mountQuerySearchById(String table, Object idValue, String idName) {
		...
		StringBuilder sql = new StringBuilder("SELECT * FROM ").append(table).append(WHERE_CLAUSE + idName + " = '")
